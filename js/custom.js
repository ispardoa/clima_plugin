jQuery(document).ready(function(){
  ...
  $(document).on('submit','#parameters_form', function(e) {
    e.preventDefault();
    $('.wqmessage').html('');
    $('.wqsubmit_message').html('');
    ...
    if(wqtitle=='') {
      $('#wqtitle_message').html('Api Key es obligatorio');
    }
    if(wqdescription=='') {
      $('#wqdescription_message').html('Descripcion es obligatoria');
    }

    if(wqtitle!='' && wqdescription!='') {
      var fd = new FormData(this);
      var action = 'wqparameters';
      fd.append("action", action);
